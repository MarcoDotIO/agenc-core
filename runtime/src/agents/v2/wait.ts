...
export function createWaitAgentTool(opts: MultiAgentV2Options): Tool {
  ...
  const execute = async (
    ...
    // Read the turn before the wait, not after it: `waitForMailboxChange`
    // takes no abort signal, so an interrupt can replace the turn while this
    // wait is still sleeping. Charging the result to whatever turn is active
    // on resume would bill the new turn for time it never spent.
